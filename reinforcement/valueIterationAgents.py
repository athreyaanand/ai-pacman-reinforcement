...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        #initialize qval to 0
        qval = 0
        #iterate through all surrounding states and associiated probs
    ...
    def computeActionFromValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        #if we're at an end we don't go anywhere :)
        if self.mdp.isTerminal(state):
          return None
        
        #initialize value to the lowest val possible and an empty policy(action to take)
        value, policy = float("-inf"), None
        #loop through possible actions
        for action in self.mdp.getPossibleActions(state):
          #assign curr qval to a temp var
          tmp = self.computeQValueFromValues(state, action)
          #if our curr qval is > our largest qval update it as our best option
          if tmp>=value:
            value = tmp
            policy = action
        return policy
